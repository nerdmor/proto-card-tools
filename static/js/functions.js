...
function translateRarity(rar){
  rar = rar.toLowerCase()
  if(window.constants.rarities.includes(rar)){
    return rar;
  }
  return 'special';
}
...
function eventFilter(evt, filter){
    if(evt.target.matches(filter)){
        return evt;
    }
    return false;
}
...
function len(obj){
    ...
    if(Object.hasOwn(obj, 'length')) return obj.length;
    if(typeof obj === 'object') return Object.entries(obj).length;
    try{
...
function isEmoji(str){
    if(/^\d+$/u.test(str)) return false;
    if(/^\P{Emoji}+$/u.test(str)) return false;
    if(/[A-z0-9\-_?\.\,\<\>\{\}\[\]\(\)]/g.test(str)) return false;
    if(/^\p{Emoji}+/u.test(str)) return true;
    return false;
}
